...
static int main_test(qspi_flash_test_ctx_t *ctx)
{
    ...
    /* Explorer boards have various flash parts, so just check that a non zero value is here for now */
    if (flash_size == 0)
    {
        local_printf("Failed flash part size incorrect.  Got %u expected != 0", flash_size);
        return -1;
    } else {
    ...
    if (page_cnt == 0)
    {
        local_printf("Failed flash part count incorrect.  Got %u expected != 0", page_cnt);
        return -1;
    } else {
